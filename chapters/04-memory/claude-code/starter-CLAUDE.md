...
Anchor in nouns that show up elsewhere in the codebase so the agent can orient fast.
...
- Never: add deps without asking, use `any`, use `// @ts-ignore`, commit secrets, skip tests.
...
Put module-specific rules in `.claude/rules/<name>.md` with `paths:` frontmatter so they only load when those files are touched. See `.claude/rules/billing.md` for an example.
